...
async def on_member_join(member):
    ...
    for i in range(0, len(invite_list)):
        ...
        if invite_list[i].uses is not data[i]:
            await secret.send(
                "{} was invited to the server by {}".format(
                    member.name, invite_list[i].inviter
                )
            )
...
async def on_message(message):
        ...
        try:
            ...
            if amount > data.get("USD"):
                await message.channel.send(
                    """Warning: Could not complete trade, \
                coin balance too low.  Your current USD balance is {:0.2f}""".format(
                        data.get("USD")
                    )
                )
                return
            if everything:
                ...
                data["USD"] = 0
                await message.channel.send(
                    "Successfully traded ${:0.2f} for {:0.2f}{}".format(
                        all, cprice, coin.upper()
                    )
                )
            else:
                data[coin] += cprice
    ...
    if message.content.lower().startswith("!csell"):
        try:
            ...
            if amount > data.get(coin):
                await message.channel.send(
                    """Warning: Could not complete trade, \
              coin balance too low.  Your current {} balance is {}""".format(
                        coin, data.get(coin)
                    )
                )
                return
    ...
    if message.content.lower().startswith("!wallet"):
        ...
        if len(cont) > 1:
            ...
            total = 0
            embed = discord.Embed(
                title="{}'s wallet".format(memb.display_name),
            ...
            total = 0
            embed = discord.Embed(
                title="{}'s wallet".format(message.author.display_name),
                color=discord.Color.blue(),
            )
            embed.set_author(
        ...
        for user in cash:
            ...
            if user == message.author.id:
                ...
                await message.channel.send(
                    "Successfully traded ${} for {}btc".format(amount, btc)
                )
                return
            else:
                i = +1
